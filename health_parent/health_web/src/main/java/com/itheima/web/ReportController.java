...
public class ReportController {
    ...
    public Result getMemberReport() {
        ...
        calendar.add(Calendar.MONTH, -12);//获得当前日期之前的12个月的日期
     ...
     * @return
     */
    @RequestMapping("/getMemberGenderReport")
    public Result getMemberGenderReport() {

        List<Map<String, Object>> list = memberService.findMemberByGender();

        if (list != null && list.size() > 0) {


            Map<String, Object> map = new HashMap<>();
            map.put("memberCountByGender", list);
    ...
    public Result getMemberAgeReport() {
        ...
        if (list != null && list.size() > 0) {
            ...
            Map<String, Object> map = new HashMap<>();
            map.put("memberCountByAge", list);

            List<String> ageRangeList = new ArrayList<>();
            ageRangeList.add("0-18");
            ageRangeList.add("18-30");
            ageRangeList.add("30-45");
            ageRangeList.add("45以上");
            map.put("ageRange", ageRangeList);
            return new Result(true, MessageConstant.GET_SETMEAL_COUNT_REPORT_SUCCESS, map);
        } else {
            return new Result(false, MessageConstant.GET_SETMEAL_COUNT_REPORT_FAIL);
        }
